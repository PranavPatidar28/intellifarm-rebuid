...
describe('StorageService', () => {
  ...
  it('authorizes assistant attachment media through thread user ownership', async () => {
    const findMany = jest
      .fn<Promise<Array<{ attachments: unknown }>>, [unknown]>()
      .mockResolvedValue([
        {
          attachments: [
            {
              type: 'image',
              url: 'http://localhost:4000/v1/media/assistant-attachments/field-view.jpg',
            },
          ],
        },
      ]);
    ...
    ).isOwnedByUser('user-id', 'assistant-attachments', 'field-view.jpg');
